...
class WernickeArea(nn.Module):
    ...
    def _rate_encode(self, embedding: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Map [-1, 1] → [0, 1] (use absolute magnitude)
        # Negative values become inhibitory contributions in Layer 2
        rates = embedding.abs() * self.max_rate
    ...
    def perceive(self, text: str) -> torch.Tensor:
        ...
        # Handle batch — take first if list
        if embedding.dim() == 2:
